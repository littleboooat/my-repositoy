...
void mytask(void *para)
{
	...
	whitelistitem item;
	uint8_t add_state;
    whitelistitem newitem={{0xE4,0x13,0x34,0x2D},"大卫",2};
    whitelistitem a={{0x34,0xCF,0X38,0X2D},"瑞恩",1};	
...
Serial_Printf("item_addr =%d\r\n", item_addr);
			...
			OLED_ShowString(3, 2, now_item.NAME);
			vTaskDelay(2000);
			OLED_ShowString(3, 1, "       ");
Serial_Printf("name=%s\r\n",now_item.NAME);
...
//Serial_Printf("DID=%x\r\n",DID);
	...
	xTaskCreate(mytask,"mytask",256,NULL,2,&mytaskhandle);
	xTaskCreate(spi_test,"mytask",128,NULL,2,&spitest_handle);
	xTaskCreate(myRTC,"myRTC",128,NULL,2,&MYRTC_handle);
	xTaskCreate(esp8266,"myesp8266",2048,NULL,2,&MYESP8266_handle);
	vTaskStartScheduler();
	while(1)
	{
