...
extern SemaphoreHandle_t MFRC522_IRQSemaphore;
void EXTI3_IRQHandler(void)
{
	if(EXTI_GetITStatus(EXTI_Line3)==SET)
	{
		...
		{	           			
				xSemaphoreGiveFromISR(MFRC522_IRQSemaphore ,NULL);	
		}
...
//		
//Serial_Printf("k=%d,irqstate=0x%x,MFR522_ReadReg(ErrorReg)=0x%02x\r\n,maxwait=%d\r\n",k,irqstate,MFR522_ReadReg(ErrorReg),maxwait);
////vTaskDelay(100)	;	
...
//	if(maxwait!=0)
//	{	
	xSemaphoreTake(MFRC522_IRQSemaphore,portMAX_DELAY);
    irqstate=MFR522_ReadReg(CommIrqReg);	
	if((irqstate&waitIrq)==0&&(irqstate&0x01)==0)
	{	
...
uint8_t MFRC522_Anticoll(uint8_t *back_datas)
{
    uint8_t senddata[2],i,check=0;
	uint16_t backlen;
	uint8_t Status=MI_ERR;
	uint8_t Command=PCD_TRANSCEIVE;	
	senddata[0]=0x93;
	senddata[1]=0x20;
	MFR522_WriteReg(BitFramingReg,0x00);
    Status=MFRC522_ToCard(Command, senddata,2 , back_datas, &backlen);
//Serial_Printf("Status1=%d\r\n",Status);
    if(Status==MI_OK)
	{
		for(i=0;i<4;i++)
		{
...
//		Serial_Printf("check=%x\r\n",check);	
		if(check!=back_datas[i]) Status=MI_ERR;
	}	
//Serial_Printf("anticoll_Status2=%d\r\n",Status);		
	return Status;
}

void MFRC522_CalculateCRC(uint8_t *buffer, uint8_t len, uint8_t *pOutData)
{
	uint8_t i,waittimes,n;
	for(uint8_t i=0;i<18;i++)
		   Serial_Printf(" writeData[%d]=%x\r\n",i,buffer[i]);
	//MFR522_WriteReg(CommandReg,PCD_IDLE);		
	MFR522_WriteReg(DivIRqReg ,0x04);
	MFRC522_SetBitMask(FIFOLevelReg ,0x80);
Serial_Printf("DivIRqReg  = %d\r\n",  MFR522_ReadReg( DivIRqReg));  

	for(i=0;i<len;i++)
	{
		MFR522_WriteReg(FIFODataReg ,buffer[i]);
	}
	

	if(len==16)
	{	
		uint8_t fifoLevel = MFR522_ReadReg(FIFOLevelReg);
        Serial_Printf("FIFOLevel = %d\r\n", fifoLevel);  // 应该是 16
    }
	MFR522_WriteReg(CommandReg,PCD_CALCCRC);
	waittimes = 1000;
	do
	{
		n= MFR522_ReadReg(DivIRqReg);
		waittimes--;
	}while((waittimes!=0)&&!(n&0x04))	;
//Serial_Printf("n=%x\r\n",n);		
	pOutData[0] = MFR522_ReadReg(CRCResultRegL);
	pOutData[1] = MFR522_ReadReg(CRCResultRegM);	
}

uint8_t MFRC522_SelectCard(uint8_t *back_datas)
{
	uint8_t buffer[9],pOutData[2],i,size;
	uint16_t backlen;
	uint8_t Status=MI_ERR;
	uint8_t Command=PCD_TRANSCEIVE;
	buffer[0]=0x93;
...
//Serial_Printf("buffer=%x\r\n",buffer[i]);	
	...
	Status=MFRC522_ToCard(Command, buffer, 9, buffer, &backlen);
//for(i=0;i<9;i++)
//	{
//		
//		Serial_Printf("buffer=%x\r\n",buffer[i]);	
//	}
//Serial_Printf("backlen=%d\r\n",backlen);		
	if(Status==MI_OK&&backlen==24) size =buffer[0];
    else size =0;
...
//	Serial_Printf("buffer[%d]=%x\r\n",j,buffer[j]);   
	Status=MFRC522_ToCard(PCD_AUTHENT, buffer, 12, buffer, &backlen);
//Serial_Printf("authStatus=%d\r\n",Status);
	if(Status!=MI_OK || (MFR522_ReadReg(Status2Reg))!=0x08)
		Status=MI_ERR;
//Serial_Printf("MFR522_ReadReg(Status2Reg)=%x\r\n",MFR522_ReadReg(Status2Reg)); 
	return Status;			
}
...
uint8_t MFRC522_WriteBlock(uint8_t blockAddr, uint8_t* writeData)
{
	...
	Status=MFRC522_ToCard( PCD_TRANSCEIVE, buffer, 4, buffer, &backlen);
Serial_Printf("writebuffer[0]=%x\r\n",buffer[0]);	
	if(Status==MI_OK && (buffer[0]&0x0F)==0x0A)
	{


		MFRC522_CalculateCRC(writeData, 16,  &writeData[16]);
//		writeData[16]=0x40;
//		writeData[17]=0x5d;
for(uint8_t i=0;i<18;i++)
		   Serial_Printf(" CRCwriteData[%x]=%x\r\n",i,writeData[i]);	
		Status=MFRC522_ToCard( PCD_TRANSCEIVE, writeData, 18, writeData, &backlen);	
		
Serial_Printf("writeData=%x,backlen=%d\r\n",writeData[0],backlen);			
		if(Status==MI_OK && (writeData[0]&0x0F)==0x0A)
			Status=MI_OK;
		else Status=MI_ERR;
	}
...
uint8_t MFRC522_Read(uint8_t blockAddr, uint8_t* buffer)
{
	...
	for(uint8_t i=0;i<4;i++)
				   Serial_Printf(" crcbuffer[%x]=%x\r\n",i,buffer[i]);	
	
	Status=MFRC522_ToCard( PCD_TRANSCEIVE, buffer, 4, buffer, &backlen);	
Serial_Printf("readStatus=%d，backlen=%d\r\n",Status,backlen);	
if(Status==MI_OK)
			{	

				for(uint8_t i=0;i<16;i++)
				   Serial_Printf(" buffer[%x]=%x\r\n",i,buffer[i]);		
			}	
	if(Status==MI_OK && backlen==18*8)
		Status=MI_OK;
	else Status=MI_ERR;
