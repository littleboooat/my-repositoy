...
//	uint8_t a[2]={0xCE ,0xD2};
	...
	while(1)
	{   
//必须用vscode将utf8转为gb2312才可以使用，因为 utf8编码下中文占三字节，gb2312是2字节。我的hzk16字库是根据gb2312来的。
//	OLED_ShowChinese(1, 1, "你");	
//    OLED_ShowChineseString(2,1,"一二三四五六七八");
//	OLED_ShowMixedString(3,1,"nihao皮");
////1.打印各个任务栈的使用情况，任务的情况
      vTaskList(pcWriteBuffer);
      UsartPrintf(USART1, "%s",pcWriteBuffer);
////2. 查看 CPU 占用时间		  
...
void Task1_Whitelist(void *para)
{
	...
	whitelistitem item;
	uint8_t add_state;
	uint8_t buffer[32];  // 用来存放一个汉字的点阵数据
	W25Q64_ReadData(0x1000+136928,buffer,32);//136928为“我”的偏移量
	for(int i=0;i<32;i++)
	{
...
//	xTaskCreate(Task0_PrintStats,"MYPrintStats",400,NULL,2,&xStatsTask);//大概使用310word。
//	xTaskCreate(Task1_Whitelist,"MYwhitelist",100,NULL,2,&xListTask);//大概使用10word。60表示60*4字节
	xTaskCreate(Task2_CardVerification,"MYcard",150,NULL,2,&xCardTask);//大概使用98word
	xTaskCreate(Task3_Time,"MYtime",100,NULL,2,&xTimeTask);//大概使用46word
