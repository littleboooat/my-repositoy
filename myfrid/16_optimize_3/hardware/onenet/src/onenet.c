...
#include "W25Q64.h"
#include "myrtc.h"
//#define PROID			"8zRVVasgeP"

//#define ACCESS_KEY		"TDk4SkFGYjNwMDQ3bkl4QlJKT1NNNW12M3dzR0J6UW0="

//#define DEVICE_NAME		"MY_DEVICE"
#define PROID			"74Kqo40mqF"
#define ACCESS_KEY		"QjZlUGxmMTNNZ2VvTjV2WUFrSDBYcHV0T0plcU0xVjQ="
...
*
*	函数功能：	计算Authorization。OneNET_Authorization 的作用就是 用 access_key 对请求信息签名，生成 OneNET 所需的鉴权 Token，确保设备在与 OneNET 平台通信时身份合法。
*
...
//
//	函数功能：	与onenet创建连接，作用：
//生成 OneNET 鉴权 Token。组装 MQTT CONNECT 报文。通过 ESP8266 发送报文到 OneNET 平台。等待平台响应并解析连接结果。返回连接状态（0 成功，1 失败）
//
//	入口参数：	无
//
//	返回参数：	1-成功	0-失败
...
_Bool OneNet_DevLink(void)
{
	...
	if(MQTT_PacketConnect(PROID, authorization_buf, DEVICE_NAME, 256, 1, MQTT_QOS_LEVEL0, NULL, NULL, 0, &mqttPacket) == 0)
	{
		...
		Serial_Printf("dataPtr=%c",dataPtr);
		if(dataPtr != NULL)
		{
...
//    memset(buf, 0, 256);            // 清空主缓存
//    memset(text, 0, sizeof(text));  // 清空text缓冲

//    // JSON 起始部分
//    strcpy(buf, "{\"id\":\"123\",\"params\":{");

//    // passEvent 节点
//    strcat(buf, "\"passEvent\":{");
...
//    sprintf(text, "\"cardId\":\"%s\",", uid_str);
//    strcat(buf, text);

//    // 添加 userId
//    memset(text, 0, sizeof(text));
//    sprintf(text, "\"userId\":\"%s\"", name_str);
//    strcat(buf, text);

//    // 关闭 value、passEvent、params
//    strcat(buf, "}}}}");

//    return strlen(buf);
//}
//==========================================================
//	函数名称：	OneNet_FillBuf(char *buf)
//
//	函数功能：	将要发送的内容转为jason格式放入buf中
//
//	入口参数：	buf
//
//	返回参数：	无
...
//	说明：		
//==========================================================
//extern QueueHandle_t name_upload;
//unsigned char OneNet_FillBuf(char *buf)
//{
//	
//    char text[128];
//    char uid_str[]="1233";
//    char name_str[]="555";
//	long long unix = 1752549841000LL;
////	whitelistitem pvBuffer;
////	xQueueReceive(name_upload, &pvBuffer, portMAX_DELAY);
////	
////	sprintf(uid_str, "%02X%02X%02X%02X", pvBuffer.UID[0], pvBuffer.UID[1], pvBuffer.UID[2], pvBuffer.UID[3]);
////	strcpy(name_str, pvBuffer.NAME);
////	

////Serial_Printf("uid=%s",uid_str);
////Serial_Printf("namemm=%s",name_str);
//    memset(buf, 0, 256);            // 清空主缓存
//    memset(text, 0, sizeof(text));  // 清空text缓冲
...
//    // 添加 userId
//    memset(text, 0, sizeof(text));
//    sprintf(text, "\"userId\":\"%s\"},", name_str);//数字要用""括起来
//    strcat(buf, text);
//	
//	memset(text, 0, sizeof(text));
//    sprintf(text, "\"time\":\"%lld\"",unix);//时间戳到毫秒级，32位系统用longlong,时间要用""括起来
//    strcat(buf, text);
//	
//   // 关闭 value、passEvent、params
//    strcat(buf, "}}}");
//Serial_Printf("buf=%s",buf);
//    return strlen(buf);
//}
//可行，包含时间戳
extern QueueHandle_t name_upload;
unsigned char OneNet_FillBuf(char *buf)
{
    ...
    char name_str[20];
	long long unix;
	unix = ((long long)myUnix-8*60*60 )* 1000;//如果不先强制转换，会直接按time_t即无符号整形来算，乘以1000会超出范围，导致错误,要上传标准时间戳即无时区偏移
	whitelistitem pvBuffer;
	xQueueReceive(name_upload, &pvBuffer, portMAX_DELAY);
	...
	sprintf(uid_str, "%02X%02X%02X%02X", pvBuffer.UID[0], pvBuffer.UID[1], pvBuffer.UID[2], pvBuffer.UID[3]);
	strcpy(name_str, pvBuffer.NAME);
Serial_Printf("myunix=%u\r\n",myUnix-8*3600);	
Serial_Printf("unix=%lld\r\n",unix);
Serial_Printf("uid=%s",uid_str);
Serial_Printf("namemm=%s",name_str);
...
//    memset(buf, 0, 256);            // 清空主缓存
	...
	memset(text, 0, sizeof(text));
    sprintf(text, "\"time\":\"%lld\"", unix);//加不加双引号都可以，关键后上传的事件时间，一定要大于等于上一次成功上传的时间
    strcat(buf, text);

    strcat(buf, "}}}");
UsartPrintf(USART1,"buf=%s",buf);
    return strlen(buf);
}
//==========================================================
//	函数名称：	OneNet_SendData
//
//	函数功能：	上传数据到平台OneNet_SendData 的主要作用是：

//从本地获取待发送的数据（通过 OneNet_FillBuf）。

//封装成 MQTT 协议报文（通过 MQTT_PacketSaveData）。

//发送给 ESP8266 模块，通过 WiFi 上传到 OneNET 平台。
