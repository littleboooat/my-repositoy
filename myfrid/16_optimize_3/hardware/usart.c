...
void gpio_PC13_Init(void)
{
	...
	GPIO_SetBits(GPIOC, GPIO_Pin_13);//初始化默认为亮，手动置1使其灭
}
...
void Usart_SendString(USART_TypeDef *USARTx, unsigned char *str, unsigned short len)
{
	...
	for(; count < len; count++)
	{
		USART_SendData(USARTx, *str++);									//注虓私邼
		while(USART_GetFlagStatus(USARTx, USART_FLAG_TC) == RESET);		//謭战注虓为詨
	}
...
void UsartPrintf(USART_TypeDef *USARTx, char *fmt,...)
{
	...
	va_start(ap, fmt);
	vsnprintf((char *)UsartPrintfBuf, sizeof(UsartPrintfBuf), fmt, ap);							//俦式郫
	va_end(ap);
