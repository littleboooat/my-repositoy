...
//MQTT_PacketConnect 函数做的事情就是：

//验证参数合法性

//根据 clean_session、遗嘱消息、用户名密码计算标志位和报文长度

//分配缓冲区存放整个 CONNECT 报文

//组装 MQTT 协议报文：

//固定头部（报文类型 + 剩余长度）

//可变头部（协议名、协议级别、标志位、KeepAlive）

//消息体（Device ID、Will、用户名、密码）
								...
								int16 send_len, int8 *type_bin_head, MQTT_PACKET_STRUCTURE *mqttPacket)
{
	...
	char topic_buf[48];
//上传属性用property,上传事件用event.属性一般是实时值。
	snprintf(topic_buf, sizeof(topic_buf), "$sys/%s/%s/thing/event/post", pro_id, dev_name);
